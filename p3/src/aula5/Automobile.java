...
public class Automobile extends Vehicle implements Motorized {
    ...
    public Automobile(int year, String plate, String color, int engineSize, int power, int consumption, Fuel fuel, int maxSpeed, int numPassengers, int weight) {
        ...
        if (weight > 3500 || weight <= 0)
            throw new IllegalArgumentException("Peso bruto inválido.");
        if (numPassengers <= 0 || numPassengers > 9)
    ...
    public boolean equals(Object v) {
        return super.equals(v) && this.year == ((Automobile)v).year() && this.plate.equals(((Automobile)v).plate()) && this.color.equals(((Automobile)v).color()) && this.engineSize == ((Automobile)v).engineSize() && this.power == ((Automobile)v).power() && this.numPassengers == ((Automobile)v).numPassengers() && this.weight == ((Automobile)v).weight() && this.category() == ((Automobile)v).category() && this.consumption == ((Automobile)v).consumption() && this.fuel == ((Automobile)v).fuel();
    }
}
